...
public abstract class Federate 
{
	...
	{		
		...
		log("Creating federate");
		rtiamb = RtiFactoryFactory.getRtiFactory().getRtiAmbassador();
		encoderFactory = RtiFactoryFactory.getRtiFactory().getEncoderFactory();
		
		log("Connect ambassador to RTI");
		fedamb = ambassador;
		...
		try 
		{
			URL[] foms = new URL[] 
			{
					(new File("msk_fom.fed").toURI().toURL())
			};
		...
		{
			log("Error while loading file: " + urle.getMessage());
			return;
		}
		...
		log("Setting the synchronization point.");
		...
		{
			log( "Cannot remove federation because it doesn't exists." );
		}
		catch( FederatesCurrentlyJoined fcj )
		{
			log( "Cannot remove federation because it's still working." );
		}
	}
